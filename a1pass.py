...
def partition_girvan_newman(graph, max_depth):
    ...
    betweenness = approximate_betweenness(H, max_depth)
...
def get_subgraph(graph, min_degree):
    ...
    #sort the list of degree
    #pick nodes whose degree is bigger than min_degree
    sg = graph.copy()

    degrees = graph.degree().items()
    degrees = sorted(degrees, key = lambda d: d[0])
    for ele in degrees:
      if ele[1] < min_degree:
        sg.remove_node(ele[0])
    return sg
    pass
...
def cut(S, T, graph):
    ...
    #cal cut
    cut = 0
    for items in edge_list:
      if items[0] in ds and items[1] in dt:
        cut += 1
      elif items[0] in dt and items[1] in ds:
        cut += 1
    return cut
    pass
...
def score_max_depths(graph, max_depths):
    ...
    ###TODO
    result = []
    for items in max_depths:
      components = partition_girvan_newman(graph, items)
      ncv = norm_cut(components[0].nodes(), components[1].nodes(), graph)
      temp = (items, ncv)
      result.append(temp)
    return result
    pass
...
def make_training_graph(graph, test_node, n):
    ...
    neighbors = training_graph.neighbors(test_node)
    neighbors = sorted(neighbors)
    remove_edges = []
    for items in neighbors:
      if len(remove_edges) >=n:
        break
      else:
        temp = (test_node, items)
        remove_edges.append(temp)
    training_graph.remove_edges_from(remove_edges)
...
def jaccard(graph, node, k):
    ...
    #graph_node = set(graph.nodes())
    #neighbors = neighbors - graph_node
    scores = []
    for n in graph.nodes():
      if n == node:
        ...
        if not((n,node) in graph.edges()) and not((node,n) in graph.edges()):
          scores.append(((node,n), 1.0 * len(neighbors2 & neighbors) / len(neighbors2 | neighbors)))
    scores = sorted(scores, key = lambda d: d[0][1])
...
def path_score(graph, root, k, beta):
    ...
    """
    ###TODO
    #get the length of shortest path from root to each node in the graph
    #get the number of pathes from root to this node
    #paths = nx.shortest_path_length(graph,node)
    #paths = sorted(paths.items, key = lambda x:x[1])
    #use neighbors to get number of pathes??
    """
    n = graph.neighbors(root) #list of neighbors of root.
    d = deque()
    d.append(root)
    num_paths = {root: 1}
    head = root
    for items in n:
      if count(items) == 0:
        num_paths[items] = num_paths[head]
    """
    #use bfs got the data
    node2distances, node2num_paths, node2parents = bfs(graph, root, 5)
    len_paths = sorted(node2distances.items(), key = lambda d: d[0])
    num_paths = sorted(node2num_paths.items(), key = lambda d: d[0])
    nodes = set(graph.nodes()) - set(graph.neighbors(root))
    result = []
    for node in nodes:
      if node != root:
        score = (beta**node2distances[node]) * node2num_paths[node]
        result.append(((root,node),score))
    result = sorted(result, key = lambda d: d[0][1])
...
def evaluate(predicted_edges, graph):
    ...
    count = 0
    for items in predicted_edges:
      if graph.has_edge(*items):
        count += 1
    return count/total
    pass
