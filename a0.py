...
def get_users(twitter, screen_names):
    ...
    """
    ###TODO
    #nvs = zip(screen_names, screen_names)
    #newDic = dict((screen_names,screen_names) for screen_names,screen_names in nvs)
...
def get_friends(twitter, screen_name):
    ...
    """
    ###TODO
    dic = {'screen_name':screen_name, 'count': 5000}
    request = robust_request(twitter, 'friends/ids', dic)
    friend = [r for r in request]
    #print('type %s' % type(friend))
    #print('length %d' %len(friend))
    friend.sort()
    return friend
    pass
...
def add_all_friends(twitter, users):
    ...
    for i in range(len(users)):
    	#print(i)
    	one_user = users[i]['screen_name']
    	#print('one_user %s' % one_user)
    	friends = get_friends(twitter, one_user)
    	#print ('friends %d' % len(friends))
...
def print_num_friends(users):
    ...
    for i in range(len(users)):
    	one_user = users[i]['screen_name']
    	print('%s: %d' % (one_user, len(users[i]['friends'])))
    pass
...
def count_friends(users):
    ...
    numbers = []
    for i in range(len(users)):
    	numbers.extend(users[i]['friends'])
    coun = Counter()
    coun.update(numbers)
    return coun
    pass
...
def friend_overlap(users):
    ...
    for i in range(0, len(users)):
    	for j in range(i+1, len(users)):
    		num = len(set(users[i]['friends']).intersection(set(users[j]['friends'])))
    		one_tuple = (users[i]['screen_name'], users[j]['screen_name'],num)
    		overlaps.append(one_tuple)
    overlaps = sorted(overlaps, key = lambda overlapss : overlapss[2], reverse = True)
    return overlaps
    pass
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    for i in range(0, len(users)):
    	if users[i]['screen_name'] == "HillaryClinton":
    		#print("test node 1")
    		listone = users[i]['friends']
    	if users[i]['screen_name'] == 'realDonaldTrump':
    		#print("test node 2")
    		listtwo = users[i]['friends']
    #print (len(listone))
    #print (len(listtwo))
    inter_list = list(set(listone).intersection(set(listtwo)))
    #print (len(inter_list))
    #user_id = inter_list[0]
    dic = {'user_id': inter_list}
    response = robust_request(twitter, 'users/lookup',dic)
    info = [r for r in response]
    return info[0]['screen_name']
    pass
...
def create_graph(users, friend_counts):
    ...
    ###TODO
    graph = nx.DiGraph()
    #graph.add_node('test point')
    for i in range(0, len(users)):
        graph.add_node(users[i]['screen_name'])
        for items in users[i]['friends']:
            if friend_counts[items]>1:
                #print(items)
                #print(friend_counts)
                graph.add_node(items)
                graph.add_edge(users[i]['screen_name'],items)
    #nx.draw(graph,arrows=False, with_labels=False)
    return graph
    pass
...
def draw_network(graph, users, filename):
    ...
    labeldic = {}
    for i in range(0, len(users)):
        labeldic[users[i]['screen_name']] = users[i]['screen_name']
    pos = nx.spring_layout(graph)
    #nx.draw_networkx_edges(graph,pos,width=0.1,alpha=0.5,style='dashed',arrows=False)
    nx.draw_networkx(graph,pos,arrows = False, labels=labeldic,alpha=0.5,node_size=90,width=0.2)
    plt.axis('off')
